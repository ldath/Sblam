...
class BayesBase {
	...
	function __construct(PDO $db, $tableprefix = 'bayes', $ignorefile = 'data/bayesignore.txt', $translateprob = 0.3) {
		...
		if ($ignorefile !== NULL) {
			if ($file = @file($ignorefile)) {
				...
				throw new Exception("Unable to load file with bayes ignored phrases $ignorefile");
		}
	...
	private function addToTranslate(array $words, array &$hashes) {
		...
		$q = "/*maxtime15*/INSERT ignore into {$this->table}translate (wordh,word) values" . str_repeat('(unhex(?),?),', $numwords - 1) . "(unhex(?),?)";
	...
	function add(array $words, $isspam, $howmuch = 1) {
		...
		if (!$this->addHashes($hashes, $isspam, $howmuch))
			return false;
	...
	private function addHashes(array $hashes, $isspam, $howmuch) {
		...
		try {
			...
			$q = "/*maxtime20*/INSERT into {$this->table}wordsh (wordh,flags," . ($isspam ? 'spam,ham' : 'ham,spam') . ") values" . str_repeat("(unhex(?),?,$howmuch,0),", count($hashes) / 2 - 1) . "(unhex(?),?,$howmuch,0) on duplicate key update added = now(), $colname = $colname+$howmuch";
			
			$this->db->prepareExecute($q, $hashes);
			if (!$this->db->exec("/*maxtime5*/UPDATE {$this->table}total set total$colname = total$colname + $howmuch"))
				throw new Exception("Can't increase counters");
			$this->db->commit();
		} catch (Exception $e) {
			$this->db->rollBack();
			throw $e;
	...
	function getTotalWords() {
		$total = $this->db->query("/*maxtime20*/SELECT count(*) as `cnt` from {$this->table}wordsh");
		if (!$total)
			return;
	...
	function getWordList(array $hashes) {
		...
		if (!$statement || !$statement->execute($hashes)) {
			return NULL;
		}
	...
	function hashWords(array $words, $addflags = false) {
		...
		foreach ($words as $w) {
			...
			$hashes[] = $h;
			if ($addflags)
				$hashes[] = (false === strpos($w, ' ')) ? 0 : 1; // mark which ones are phrases
			else
				self::$dictionary[strtolower($h)] = $w;
		}
		return $hashes;
	}
	
	protected function hashToWord($hash) {
		if (isset(self::$dictionary[strtolower($hash)]))
			return 'h:' . self::$dictionary[strtolower($hash)];
		
		$sta = $this->db->prepare("/*maxtime=2*/SELECT word FROM {$this->table}translate WHERE wordh=unhex(?) LIMIT 1");
		
		if ($sta && $sta->execute(array($hash)))
			foreach ($sta as $word)
				return 'H:' . $word['word'];
		return '?' . $hash;
	}
	...
	function testWordsPornelsWay(array $words) {
		...
		$totalspam /= 2;
		$totalham /= 2; // these are too inflated!
		

		$wordlist = $this->getWordList($this->hashWords($words));
		if (!$wordlist)
			return;
		$words = NULL;
		...
		foreach ($wordlist as $r) {
			...
			if (($spam + $ham) > 1 && abs($spam - $ham) < 0.3)
				continue; // noise?
			

			$bonus = 2;
			// extra penalty for very spammy words
			if ($spam > 7 && $ham < $spam / 11) {
				$ham /= 2;
				$spam = 150 - (150 - $spam) * 0.8;
				$bonus += 5;
			}
			
			if ($r['ham'] < 1 && $r['spam'] > 20)
				$bonus += min(10, 2 + $spam); // totally spammy words
			if ($r['flags'] & 1)
				$bonus += 5 + $bonus / 2; // matching phrases is more precise, score higher
			...
			if ($nudge > $spammiestwordnudge) {
				$spammiestwordnudge = $nudge;
				$spammiestwordh = $r['wordh'];
			} // just for fun
